...
TOTAL_TOKENS = 533_978_966  # example total tokens overall (adjust as needed)
EN_TOKENS = int(TOTAL_TOKENS * 0.2)  # 20% English
...
if __name__ == "__main__":
    ...
    en_dataset = load_dataset("roneneldan/TinyStories", num_proc=num_proc)["train"]

    print("Loading other languages (CausalNLP)...")
    other_dataset = load_dataset("CausalNLP/multilingual_tinystories_data_2", num_proc=num_proc)
    ...
    output_dir = "./val_data_per_lang_tinystories"
    ...
    for lang, target_tokens in val_targets.items():
        ...
        val_path = os.path.join(output_dir, f"{lang}_tinystories_val.bin")
