...
def get_val_batch_from_file(bin_path):
...
def estimate_loss():
    ...
    # evaluate per-language validation
    for lang, path in val_langs.items():
        losses = torch.zeros(eval_iters)
        for k in range(eval_iters):
            X, Y = get_val_batch_from_file(path)
            with ctx:
                _, loss = model(X, Y)
            losses[k] = loss.item()
        out[f'val/{lang}'] = losses.mean()
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if wandb_log:
            ...
            for lang in val_langs:
                log_dict[f"val/loss_{lang}"] = losses[f'val/{lang}']
            wandb.log(log_dict)
